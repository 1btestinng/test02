...
export default function AdvancedFinancialChart({points,interactionPoints=points,localCurrency,fxRate,marketCap=false}:Props){
  ...
  const logMin=effectiveScale==='log'?Math.log10(min):0;
  const logMax=effectiveScale==='log'?Math.log10(max):1;
  const y=(value:number)=>{if(effectiveScale==='log'){const lv=Math.log10(Math.max(value,min));return height-padY-((lv-logMin)/(logMax-logMin||1))*plotHeight;}return height-padY-((value-min)/spread)*plotHeight;};
  ...
  const first=displayInteraction[0];
  const last=displayInteraction.at(-1);
  const change=first&&last&&first.value!==0?((last.value-first.value)/Math.abs(first.value))*100:0;
  const selectAtClientX=(clientX:number)=>{
    const svg=svgRef.current;if(!svg||!first||!last)return;
    const rect=svg.getBoundingClientRect();if(rect.width<=0)return;
    const localX=Math.max(0,Math.min(rect.width,clientX-rect.left));
    const chartX=(localX/rect.width)*width;
    const ratio=Math.max(0,Math.min(1,(chartX-padX)/plotWidth));
    const firstTime=new Date(first.date).getTime(),lastTime=new Date(last.date).getTime();
    const targetTime=firstTime+ratio*(lastTime-firstTime);
    const index=nearestPoint(displayInteraction,targetTime);
    const point=displayInteraction[index];
    const pointTime=new Date(point.date).getTime();
    const dateRatio=lastTime===firstTime?0:(pointTime-firstTime)/(lastTime-firstTime);
    const px=padX+Math.max(0,Math.min(1,dateRatio))*plotWidth;
    const py=y(point.value);
    setSelection({point,left:(px/width)*rect.width,top:(py/height)*rect.height});
  };
  ...
  return <div className="realChartWrap" style={{position:'relative',touchAction:'pan-y'}}>
    <div className="chartControlBar" aria-label={`${marketCap?'Market capitalization':'Price'} chart controls`}>
      <div className="chartControlCluster"><span className="chartControlLabel">Currency</span><div className="chartToggle" role="group" aria-label="Chart currency"><button type="button" className={chartCurrency==='local'?'active':''} onClick={()=>setChartCurrency('local')}>{localCurrency}</button><button type="button" className={chartCurrency==='usd'?'active':''} disabled={!canUseUsd} onClick={()=>setChartCurrency('usd')}>USD</button></div>{!canUseUsd&&<span className="chartControlHint">USD unavailable</span>}</div>
      <div className="chartControlCluster"><span className="chartControlLabel">Scale</span><div className="chartToggle" role="group" aria-label="Chart scale"><button type="button" className={effectiveScale==='linear'?'active':''} onClick={()=>setScale('linear')}>Linear</button><button type="button" className={effectiveScale==='log'?'active':''} disabled={!logAvailable} onClick={()=>setScale('log')}>Log</button></div>{!logAvailable&&<span className="chartControlHint">Positive values required</span>}</div>
    </div>
    ...
    <div style={{position:'relative'}}>
      <svg ref={svgRef} className="realChart" style={{width:'100%',height:330,display:'block',cursor:'crosshair'}} viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Historical ${marketCap?'market capitalization':'price'} chart in ${displayCurrency} on ${effectiveScale} scale`} onPointerMove={handlePointerMove} onPointerDown={event=>selectAtClientX(event.clientX)} onPointerLeave={()=>setSelection(null)}>
      ...
      {selection&&<div role="status" aria-live="polite" style={{position:'absolute',left:Math.max(6,Math.min(selection.left-78,Math.max(6,(svgRef.current?.clientWidth??0)-178))),top:Math.max(6,Math.min(selection.top-92,238)),minWidth:166,pointerEvents:'none',background:'var(--panel)',border:'1px solid var(--line)',borderRadius:8,boxShadow:'0 6px 20px rgba(0,0,0,.10)',padding:'9px 11px',zIndex:3,fontSize:11,lineHeight:1.45}}><div style={{color:'var(--muted)',fontSize:10,marginBottom:4}}>{formatDate(selection.point.date)}</div><strong className="mono" style={{fontSize:13}}>{marketCap?'Market Cap':'Price'}&nbsp;&nbsp;{marketCap?compact(selection.point.value,displayCurrency):`${displayCurrency==='USD'?'$':displayCurrency+' '}${fmt(selection.point.value)}`}</strong></div>}
    </div>
    ...
    {chartCurrency==='usd'&&canUseUsd&&<div className="dataNote">USD values use the market's configured verified FX rate ({localCurrency}/USD {fxRate}). No historical FX series is connected, so this is a current/configured-FX conversion rather than a historical FX-adjusted series.</div>}
    {scale==='log'&&!logAvailable&&<div className="dataNote">Log scale requires all visible observations to be strictly positive.</div>}
  </div>;
}
