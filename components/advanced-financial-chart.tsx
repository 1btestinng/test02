...
type Props={points:ChartPoint[];interactionPoints?:ChartPoint[];localCurrency:string;marketCap?:boolean;fxSource?:string;fxCoverageStart?:string;fxCoverageEnd?:string};
...
export default function AdvancedFinancialChart({points,interactionPoints=points,localCurrency,marketCap=false,fxSource,fxCoverageStart,fxCoverageEnd}:Props){
  const [chartCurrency,setChartCurrency]=useState<ChartCurrency>('local');
  const [scale,setScale]=useState<ChartScale>('linear');
  const [selection,setSelection]=useState<Selection|null>(null);
  const svgRef=useRef<SVGSVGElement|null>(null);
  const usdPoints=useMemo(()=>points.map(p=>{const rate=p.fxRate;return rate!==undefined&&Number.isFinite(rate)&&rate>0?{date:p.date,value:p.value/rate,fxRate:rate}:null}).filter((p):p is ChartPoint=>p!==null),[points]);
  const usdInteraction=useMemo(()=>interactionPoints.map(p=>{const rate=p.fxRate;return rate!==undefined&&Number.isFinite(rate)&&rate>0?{date:p.date,value:p.value/rate,fxRate:rate}:null}).filter((p):p is ChartPoint=>p!==null),[interactionPoints]);
  const canUseUsd=usdPoints.length>=2&&usdInteraction.length>=2;
  const displayCurrency=chartCurrency==='usd'?'USD':localCurrency;
  const displayPoints=chartCurrency==='usd'&&canUseUsd?usdPoints:points;
  const displayInteraction=chartCurrency==='usd'&&canUseUsd?usdInteraction:interactionPoints;
  ...
  return <div className="realChartWrap" style={{position:'relative',touchAction:'pan-y'}}>
    ...
    {chartCurrency==='usd'&&canUseUsd&&<div className={styles.note}>USD values use point-in-time historical FX rates aligned to each market observation date.{fxSource?` Source: ${fxSource}.`:''}{fxCoverageNote?` ${fxCoverageNote}.`:''}</div>}
  </div>;
}
