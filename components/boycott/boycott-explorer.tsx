...
function Logo({entry}:{entry:BoycottEntry}){
  ...
  const resolve=async()=>{
    ...
    try{
      const response=await fetch(`/api/logos?name=${encodeURIComponent(entry.company)}`,{cache:'force-cache'});
      ...
      setSrc(`https://cdn.tickerlogos.com/${data.website.replace(/^https?:\/\//,'').replace(/\/$/,'')}`);
    }catch{setFailed(true);}
  };
  return <span className={styles.logo}>{!failed&&src?<img src={src} alt="" loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={()=>{setSrc('');setFailed(true)}}/>:<span aria-hidden="true">{initials(entry.company)}</span>}{!src&&!failed&&<span className={styles.logoProbe} aria-hidden="true" ref={node=>{if(node)void resolve()}}/>}</span>;
}
  ...
  return <div className={styles.explorer}>
    <div className={styles.searchPanel}>
      <div className={styles.searchBox}>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search company or product..." aria-label="Search company or product" />
        {search&&<button className={styles.clearSearch} onClick={()=>setSearch('')} aria-label="Clear search">×</button>}
      </div>
      <div className={styles.filterRow}>
        <div className={styles.categoryNav} role="tablist" aria-label="Quick categories">
          <button className={category==='all'?styles.activeTab:''} onClick={()=>setCategory('all')} role="tab" aria-selected={category==='all'}>All</button>
          {categories.slice(0,7).map(value=><button key={value} className={category===value?styles.activeTab:''} onClick={()=>setCategory(value as BoycottCategory)} role="tab" aria-selected={category===value}>{CATEGORY_LABELS[value as BoycottCategory]}</button>)}
        </div>
      ...
      {showFilters&&<div className={styles.filterPanel}>
        <label>Category<select value={category} onChange={e=>setCategory(e.target.value as BoycottCategory|'all')}><option value="all">All categories</option>{categories.map(value=><option key={value} value={value}>{CATEGORY_LABELS[value as BoycottCategory]}</option>)}</select></label>
        <label>Campaign<select value={campaign} onChange={e=>setCampaign(e.target.value)}><option value="all">All campaigns</option>{campaigns.map(value=><option key={value} value={value}>{value}</option>)}</select></label>
        ...
        {active&&<button className={styles.clearFilters} onClick={clear}>Clear all</button>}
      </div>}
