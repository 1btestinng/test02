...
export default function PeopleTable({initialRows}:{initialRows:RankedPerson[]}) {
  ...
  async function vote(personId:string,vote:'like'|'dislike'|'none') {
    ...
    setPending(personId);setError(null);
    try{
      ...
      if(!response.ok){const body=await response.json().catch(()=>null) as {error?:string}|null;throw new Error(body?.error??'Vote failed');}
      ...
      router.refresh();
    }catch(caught){setError(caught instanceof Error?caught.message:'Voting is temporarily unavailable.');}
