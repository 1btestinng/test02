...
export default function NorthAfricaShell({children}:{children:React.ReactNode}){
 const pathname=usePathname();const [collapsed,setCollapsed]=useState(false);const [mobileOpen,setMobileOpen]=useState(false);const [customizing,setCustomizing]=useState(false);const [order,setOrder]=useState<string[]>(DEFAULT_NAVIGATION_ORDER);const [hydrated,setHydrated]=useState(false);
 ...
 return <div className={`${styles.frame}${collapsed?` ${styles.collapsed}`:''}`}>
  ...
  {customizing&&<div className={styles.modalBackdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="naCustomizeTitle"><div className={styles.modalHead}><div><div className="eyebrow">Navigation</div><h2 id="naCustomizeTitle">Customize navigation</h2><p>Reorder sections for this browser. Your preference stays local to this device.</p></div><button className={styles.close} onClick={()=>setCustomizing(false)} aria-label="Close">×</button></div><div className={styles.list}>{items.map((item,i)=><div className={styles.row} key={item.id}><span className={styles.drag} aria-hidden="true">☰</span><span>{item.label}</span><div className={styles.moves}><button onClick={()=>move(item.id,-1)} disabled={i===0} aria-label={`Move ${item.label} up`}>↑</button><button onClick={()=>move(item.id,1)} disabled={i===items.length-1} aria-label={`Move ${item.label} down`}>↓</button></div></div>)}</div><div className={styles.foot}><button className={styles.secondary} onClick={()=>setOrder(DEFAULT_NAVIGATION_ORDER)}>Reset</button><button className={styles.primary} onClick={()=>setCustomizing(false)}>Done</button></div></section></div>}
 </div>;
}
