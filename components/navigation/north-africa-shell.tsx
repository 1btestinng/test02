...
export default function NorthAfricaShell({children}:{children:React.ReactNode}){
 ...
 return <div className={`${styles.frame}${collapsed?` ${styles.collapsed}`:''}`}>
  <aside className={`${styles.sidebar}${mobileOpen?` ${styles.open}`:''}`} aria-label="North Africa Hub navigation">
   <div className={styles.top}><Link href="/" className={styles.brand} aria-label="North Africa Hub home"><span className={styles.brandLine}>North Africa</span><span className={styles.brandLine}>Hub</span></Link><button className={styles.collapse} onClick={()=>setCollapsed(v=>!v)} aria-label={collapsed?'Expand navigation':'Collapse navigation'} title={collapsed?'Expand navigation':'Collapse navigation'}>{collapsed?'→':'←'}</button></div>
  ...
  {mobileOpen&&<button className={styles.overlay} aria-label="Close navigation" onClick={()=>setMobileOpen(false)}/>}<div className={styles.content}><button className={styles.mobileMenu} onClick={()=>setMobileOpen(true)} aria-label="Open North Africa Hub navigation"><span/><span/><span/></button>{children}</div>
