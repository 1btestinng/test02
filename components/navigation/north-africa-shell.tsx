...
export default function NorthAfricaShell({children}:{children:React.ReactNode}){
 ...
 return <div className={`${styles.frame}${collapsed?` ${styles.collapsed}`:''}`}>
  <aside className={`${styles.sidebar}${mobileOpen?` ${styles.mobileOpen}`:''}`} aria-label="North Africa navigation">
   ...
   <nav className={styles.nav} aria-label="Primary navigation">
    {(['explore','data','about'] as const).map(group=><div className={styles.group} key={group}><div className={styles.label}>{group==='explore'?'Explore':group==='data'?'Data':'About'}</div>{items.filter(x=>x.group===group).map(item=><Link key={item.id} href={item.href} className={`${styles.item}${active(item.href,item.id)?` ${styles.active}`:''}`} title={collapsed?item.label:undefined}><Icon id={item.id}/><span>{item.label}</span></Link>)}</div>)}
   </nav>
  ...
  {mobileOpen&&<button className={styles.overlay} aria-label="Close navigation" onClick={()=>setMobileOpen(false)}/>} 
  <div className={styles.content}>
   <button className={styles.mobileMenu} onClick={()=>setMobileOpen(true)} aria-label="Open navigation"><span/><span/><span/></button>
   {children}
  </div>
  {customizing&&<div className={styles.modalBackdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="naCustomizeTitle">
    <div className={styles.modalHead}><div><div className="eyebrow">Navigation</div><h2 id="naCustomizeTitle">Customize navigation</h2><p>Reorder sections for this browser. Your preference stays local to this device.</p></div><button className={styles.close} onClick={()=>setCustomizing(false)} aria-label="Close">×</button></div>
    <div className={styles.list}>{items.map((item,i)=><div className={styles.row} key={item.id}><span className={styles.drag} aria-hidden="true">☰</span><span>{item.label}</span><div className={styles.moves}><button onClick={()=>move(item.id,-1)} disabled={i===0} aria-label={`Move ${item.label} up`}>↑</button><button onClick={()=>move(item.id,1)} disabled={i===items.length-1} aria-label={`Move ${item.label} down`}>↓</button></div></div>)}</div>
    <div className={styles.foot}><button className={styles.secondary} onClick={reset}>Reset</button><button className={styles.primary} onClick={()=>setCustomizing(false)}>Done</button></div>
  </section></div>}
 </div>
}
