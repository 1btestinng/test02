...
const SEED_ENTRIES: BoycottEntry[] = [
  {
    id: 'carrefour', rank: 1, company: 'Carrefour', product: 'Carrefour', category: 'grocery',
    reason: 'Targeted in documented Palestine-related boycott campaigns; BDS identifies Carrefour as a priority target.',
    status: 'Documented campaign', source: 'BDS / North Africa campaigns', sourceUrl: 'https://bdsmovement.net/Guide-to-BDS-Boycott', confidence: 'high', campaignType: 'BDS priority / regional campaign',
    aliases: ['Carrefour Market'],
  },
  {
    id: 'mcdonalds', rank: 2, company: "McDonald's", product: "McDonald's", category: 'food',
    ...
    status: 'Documented campaign', source: 'BDS / regional campaigns', sourceUrl: 'https://bdsmovement.net/Guide-to-BDS-Boycott', confidence: 'high', campaignType: 'BDS priority / regional campaign',
    aliases: ['McD', 'McDonalds'],
  },
];
...
export const CATEGORY_LABELS: Record<BoycottCategory,string> = {
  food: 'Food & Restaurants', beverages: 'Beverages', grocery: 'Grocery & Supermarkets', fashion: 'Fashion & Clothing',
  beauty: 'Beauty & Personal Care', household: 'Household', baby: 'Baby & Family', technology: 'Technology',
  software: 'Software & Internet', finance: 'Finance & Banking', travel: 'Travel & Hospitality', automotive: 'Transportation & Automotive',
  energy: 'Energy', media: 'Entertainment & Media', healthcare: 'Healthcare & Pharmaceuticals', industrial: 'Industrial',
  construction: 'Construction & Real Estate', agriculture: 'Agriculture', logistics: 'Logistics', retail: 'Retail', other: 'Other',
};
...
const CATEGORY_RULES: Array<[BoycottCategory,RegExp]> = [
  ...
  ['automotive', /toyota|volkswagen|ford|jaguar|land rover|mitsubishi|general motors|hyundai|automotive|car/i],
  ['energy', /bp|chevron|caltex|exxon|mobil|solar|energix|renewable|energy|oil|gas/i],
  ['media', /disney|espn|politico|netflix|spotify|youtube|media|broadcast|publishing/i],
...
function cleanCompanyName(value:string){
  return value.replace(/\s*Company\s*$/i,'').replace(/\s*\([^)]*\)\s*$/g,'').replace(/Company$/i,'').trim();
}
...
function inferCategory(company:string):BoycottCategory{
  for(const [category,pattern] of CATEGORY_RULES){ if(pattern.test(company)) return category; }
  return 'other';
}
...
function aliasesFor(company:string):string[]{
  const aliases:Record<string,string[]> = {
    ...
    'Google':['YouTube'], 'P&G':['Procter & Gamble'], 'L’Oréal':['L Oreal','L'Oreal'], 'L\'Oréal':['L Oreal','L’Oreal'],
  };
...
export async function getBoycottEntries():Promise<BoycottEntry[]>{
  try{
    ...
    for(const match of matches){
      ...
      sourceEntries.push({
        id:`source-${slugify(company)}`, rank:0, company, product:company, category:inferCategory(company), aliases:aliasesFor(company),
        ...
        status:'Source database entry', source:SOURCE_LABEL, sourceUrl:SOURCE_URL, confidence:'medium', campaignType:'Third-party research database',
      });
    }
