...
function aliasesFor(company:string):string[]{const aliases:Record<string,string[]>={'Coca-Cola':['Coke','Coca Cola'],"McDonald's":['McDonalds','McD'],Facebook:['Meta'],Meta:['Facebook','Instagram','WhatsApp'],Google:['YouTube'],'P&G':['Procter & Gamble'],"L’Oréal":['L Oreal',"L'Oreal"],"L'Oréal":['L Oreal','L’Oréal']};return aliases[company]??[];}
...
export async function getBoycottEntries():Promise<BoycottEntry[]>{
  try{
    ...
    const sourceEntries:BoycottEntry[]=[];const seen=new Set(SEED_ENTRIES.map(entry=>entry.company.toLowerCase()));
    for(const match of matches){const raw=match[1].replace(/<[^>]+>/g,' ').replace(/&amp;/g,'&').replace(/&#39;/g,"'").replace(/&quot;/g,'"').replace(/\s+/g,' ').trim();const company=cleanCompanyName(raw);if(!company||company.length<2)continue;const key=company.toLowerCase();if(seen.has(key))continue;seen.add(key);sourceEntries.push({id:`source-${slugify(company)}`,rank:0,company,product:company,category:inferCategory(company),aliases:aliasesFor(company),reason:'See cited source for the documented boycott rationale.',status:'Source database entry',source:SOURCE_LABEL,sourceUrl:SOURCE_URL,confidence:'medium',campaignType:'Third-party research database'});}
