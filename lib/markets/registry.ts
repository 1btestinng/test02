...
const egyptProvider:MarketDataProvider={
  async getCompanies(){return egyptCompanies();},async getCompany(ticker){return egyptCompanies().find(c=>c.ticker.toUpperCase()===ticker.toUpperCase());},
  async getMarketSummary(){return summarize(egyptCompanies(),EGYPT_CONFIG,51.36,'Configured EGP/USD snapshot');},async getFX(){return 51.36;},async getMarketStatus(){return getMarketStatusSync('EG');}
};
const moroccoProvider:MarketDataProvider={
  async getCompanies(){return moroccoCompaniesSnapshot();},async getCompany(ticker){return moroccoCompaniesSnapshot().find(c=>c.ticker.toUpperCase()===ticker.toUpperCase());},
  async getMarketSummary(){return summarize(moroccoCompaniesSnapshot(),MOROCCO_CONFIG,MOROCCO_FX_USD_MAD,'USD/MAD market snapshot');},async getFX(){return MOROCCO_FX_USD_MAD;},async getMarketStatus(){return getMarketStatusSync('MA');}
};
const tunisiaProvider:MarketDataProvider={
  async getCompanies(){return tunisiaCompaniesSnapshot();},async getCompany(ticker){return tunisiaCompaniesSnapshot().find(c=>c.ticker.toUpperCase()===ticker.toUpperCase());},
  async getMarketSummary(){return summarize(tunisiaCompaniesSnapshot(),TUNISIA_CONFIG,TUNISIA_FX_USD_TND,'USD/TND market snapshot');},async getFX(){return TUNISIA_FX_USD_TND;},async getMarketStatus(){return getMarketStatusSync('TN');}
};
const algeriaProvider:MarketDataProvider={
  async getCompanies(){return algeriaCompaniesSnapshot();},async getCompany(ticker){return algeriaCompaniesSnapshot().find(c=>c.ticker.toUpperCase()===ticker.toUpperCase());},
  async getMarketSummary(){return summarize(algeriaCompaniesSnapshot(),ALGERIA_CONFIG,ALGERIA_FX_USD_DZD,'USD/DZD market snapshot');},async getFX(){return ALGERIA_FX_USD_DZD;},async getMarketStatus(){return getMarketStatusSync('DZ');}
};
...
export const MARKET_REGISTRY:Record<string,{config:MarketConfig;provider:MarketDataProvider}>={
  ...
  TN:{config:TUNISIA_CONFIG,provider:tunisiaProvider},
  DZ:{config:ALGERIA_CONFIG,provider:algeriaProvider}
};
...
export function getMarketCompaniesSync(code:string){
  switch(code.toUpperCase()){
    case 'MA':return moroccoCompaniesSnapshot();
    case 'TN':return tunisiaCompaniesSnapshot();
    case 'DZ':return algeriaCompaniesSnapshot();
    default:return egyptCompanies();
  }
}
...
export function getMarketSummarySync(code:string){
  const normalized=code.toUpperCase();
  const market=getMarket(normalized);
  const fx=normalized==='MA'?MOROCCO_FX_USD_MAD:normalized==='TN'?TUNISIA_FX_USD_TND:normalized==='DZ'?ALGERIA_FX_USD_DZD:51.36;
  const source=normalized==='MA'?'USD/MAD market snapshot':normalized==='TN'?'USD/TND market snapshot':normalized==='DZ'?'USD/DZD market snapshot':'Configured EGP/USD snapshot';
  return summarize(getMarketCompaniesSync(normalized),market.config,fx,source);
}
export function getMarketStatusSync(code:string):'open'|'closed'|'auction'{
  const normalized=code.toUpperCase();
  const config=getMarket(normalized).config;
  ...
  const weekend=normalized==='MA'||normalized==='TN'?weekday==='Sat'||weekday==='Sun':weekday==='Fri'||weekday==='Sat';
  const open=normalized==='MA'?570:normalized==='TN'?540:600;
  const close=normalized==='MA'?930:normalized==='TN'?900:870;
