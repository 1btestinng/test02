...
function providerSymbol(code:GulfCode,ticker:string,exchangeCode:string){
  if(code==='AE')return `${ticker}${exchangeCode==='ADX'?UAE_SUFFIX.ADX:UAE_SUFFIX.DFM}`;
  return `${ticker}${SUFFIX[code]??''}`;
}
...
async function fetchQuotes(symbols:string[]):Promise<Map<string,YahooQuote>>{
  ...
  try{
    ...
    // Fall through to the chart endpoint below.
  }
  return result;
}
...
async function fetchChartQuote(symbol:string):Promise<YahooQuote|undefined>{
  try{
    ...
    const payload=await response.json() as {chart?:{result?:Array<{meta?:{regularMarketPrice?:number;previousClose?:number;regularMarketTime?:number;currency?:string};indicators?:{quote?:Array<{close?:Array<number|null>}>}}>}};
...
export async function getLiveGulfCompanies(code:GulfCode,seeds:Seed[],currency:string,localPerUsd:number):Promise<MarketCompany[]>{
  ...
  if(missing.length){
    const fallback=await Promise.all(missing.map(async item=>[item.providerSymbol,await fetchChartQuote(item.providerSymbol)] as const));
    for(const [symbol,quote] of fallback)if(quote)quotes.set(symbol,quote);
  }
