...
async function fetchQuotes(symbols: string[]): Promise<Map<string, YahooQuote>> {
  ...
  try {
    ...
    // Chart endpoint below provides the price fallback when quote is unavailable.
  }
...
async function fetchChartQuote(symbol: string): Promise<YahooQuote | undefined> {
  try {
    ...
    const price = finite(item.meta?.regularMarketPrice)
      ? item.meta.regularMarketPrice
      : closes[closes.length - 1];
    const previousClose = finite(item.meta?.previousClose)
      ? item.meta.previousClose
      : closes.length > 1
    ...
    return {
      ...
      regularMarketTime: item.meta?.regularMarketTime,
  ...
  if (missing.length) {
    const fallback = await Promise.all(
      missing.map(async (item) => [
        item.providerSymbol,
        await fetchChartQuote(item.providerSymbol),
      ] as const),
    );
