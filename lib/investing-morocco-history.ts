...
function marketMatch(company:MarketCompany,quote:SearchQuote){
  ...
  const exchange=normalizeText(quote.exchange??'');
...
function scoreQuote(company:MarketCompany,quote:SearchQuote){
  const ticker=company.ticker.trim().toUpperCase(),name=normalizeText(company.name),description=normalizeText(quote.description??''),symbol=(quote.symbol??'').toUpperCase();
  let score=0;if(symbol===ticker)score+=100;if(marketMatch(company,quote))score+=80;if(description===name)score+=30;
  const firstName=name.split(' ')[0];if(firstName&&description.includes(firstName))score+=10;
  if(quote.type?.toLowerCase().includes('equity'))score+=5;return score;
}
async function getJson<T>(url:string){
  const response=await fetch(url,{headers:{Accept:'application/json, text/plain, */*','User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/153.0.0.0 Safari/537.36','domain-id':'www',Referer:'https://www.investing.com/'},next:{revalidate:3600}});
  if(!response.ok)throw new Error(`${SOURCE} returned HTTP ${response.status}`);return (await response.json()) as T;
}
async function resolvePairId(company:MarketCompany){
  ...
  const marketCandidates=candidates.filter(q=>marketMatch(company,q));
  const ranked=(marketCandidates.length?marketCandidates:candidates).sort((a,b)=>scoreQuote(company,b)-scoreQuote(company,a));
  const best=ranked[0],id=best?quoteId(best):undefined;if(!id)throw new Error(`${SOURCE} instrument was not found for ${company.countryCode} ${company.ticker}`);return {id};
}
function parseTimestamp(value:unknown){
  const n=asNumber(value);if(n!==undefined){const date=new Date(n>10_000_000_000?n:n*1000);if(!Number.isNaN(date.getTime()))return date.toISOString();}
  const text=asString(value);if(text){const date=new Date(text);if(!Number.isNaN(date.getTime()))return date.toISOString();}return undefined;
}
...
export async function getInvestingMoroccoHistoricalPrices(company:MarketCompany){
  ...
  try{
    ...
    for(let request=0;request<MAX_REQUESTS&&end>earliest;request++){
      const start=new Date(end);start.setDate(start.getDate()-WINDOW_DAYS);if(start<earliest)start.setTime(earliest.getTime());
      const rows=await fetchRange(id,start.toISOString().slice(0,10),end.toISOString().slice(0,10));if(rows.length)points.push(...rows);
      if(start<=earliest)break;end=new Date(start);end.setDate(end.getDate()-1);
    }
    const history=merge(points);if(history.length<2)throw new Error(`${SOURCE} returned fewer than two historical observations for ${company.countryCode} ${company.ticker}`);
