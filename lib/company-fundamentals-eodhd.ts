...
async function resolveExchange(countryCode:string):Promise<string|undefined>{
  ...
  const match=exchanges.find(x=>x.CountryISO2?.toUpperCase()===countryCode.toUpperCase());
...
async function resolveTicker(company:MarketCompany):Promise<string|undefined>{
  ...
  const explicit=process.env[`EODHD_${company.countryCode.toUpperCase()}_EXCHANGE`];
  const exchange=await resolveExchange(company.countryCode);if(!exchange)return undefined;
  const configuredTicker=process.env[`EODHD_${company.countryCode.toUpperCase()}_${company.ticker.toUpperCase().replace(/[^A-Z0-9]/g,'_')}`];
  ...
  if(explicit&&exact)return `${exact.Code}.${exchange}`;
...
export async function getEodhdCompanyFundamentals(company:MarketCompany):Promise<CompanyFinancialData|undefined>{
  ...
  try{
    ...
    const epsTTM=num(highlights.EarningsShare);
    const shares=num(sharesStats.SharesOutstanding);
    const cashValue=num(latestBalance?.cash);
    ...
    const revenueGrowth=records(income.quarterly).length>=5?((num(records(income.quarterly).sort((a,b)=>String(b.date??'').localeCompare(String(a.date??'')))[0]?.totalRevenue)??0)/(num(records(income.quarterly).sort((a,b)=>String(b.date??'').localeCompare(String(a.date??'')))[4]?.totalRevenue)??0)-1):undefined;
    const roe=equity&&equity!==0&&netIncomeTTM!==undefined?netIncomeTTM/equity:undefined;
    const roa=assets&&assets!==0&&netIncomeTTM!==undefined?netIncomeTTM/assets:undefined;
    const fcfMargin=fcfTTM!==undefined&&revenueTTM!==undefined&&revenueTTM!==0?fcfTTM/revenueTTM:undefined;
    const bookValuePerShare=shares&&equity?equity/shares:undefined;
