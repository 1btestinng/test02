...
export async function generateMetadata({params}:{params:Promise<{segments:string[]}>}){const resolved=resolveSegments((await params).segments);if(!resolved)return {title:'Company · Koshary and Couscous'};const market=getMarket(resolved.country);const company=getMarketCompanySync(resolved.country,resolved.ticker);return {title:company?`${company.name} (${company.ticker}) · ${market.config.countryName} · Koshary and Couscous`:`Company · ${market.config.countryName} · Koshary and Couscous`,description:company?`${company.name} (${company.ticker}) stock price, historical prices, market capitalization and financial information on Koshary and Couscous.`:undefined};}
...
export default async function CompanyPage({params}:{params:Promise<{segments:string[]}>}){
  ...
  return <div className="shell"><header className="header"><Link href={`/markets?country=${market.config.countryCode}`} className="brand">Koshary and Couscous</Link><Link href={`/markets?country=${market.config.countryCode}`} className="muted" style={{fontSize:12}}>Back to Stock Market</Link></header><main className="detail"><CompanyIntelligence company={company} history={history} financials={financials} exchangeName={market.config.exchangeName} countryName={`${market.config.flag} ${market.config.countryName}`} currency={market.config.currencyCode} fxRate={fxRate} lastUpdated={lastUpdated} dataSource={marketData.source} delay={marketData.delay} providerTicker={providerTicker} dataError={marketData.error??historyError}/><p className="sub" style={{marginTop:24,lineHeight:1.7}}>Quote/history source: {marketData.source}. Fundamental source: {fundamentalSource}. {providerTicker?`Market-data symbol: ${providerTicker}. `:''}Historical chart range uses the full verified provider history when available. Current FX reference: {fxRate!==undefined?`${fxRate.toLocaleString('en-US',{maximumFractionDigits:4})} ${market.config.currencyCode}/USD.`:'unavailable'}</p></main></div>;
}
