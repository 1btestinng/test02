...
export default async function CompanyPage({params}:{params:Promise<{segments:string[]}>}){
  ...
  return <div className="shell"><header className="header"><Link href={`/?country=${market.config.countryCode}`} className="brand">iStocks</Link><Link href={`/?country=${market.config.countryCode}`} className="muted" style={{fontSize:12}}>Back to ranking</Link></header><main className="detail"><CompanyIntelligence company={company} history={history} financials={financials} exchangeName={market.config.exchangeName} countryName={`${market.config.flag} ${market.config.countryName}`} currency={market.config.currencyCode} fxRate={fxRate} lastUpdated={lastUpdated} dataSource={marketData.source} delay={marketData.delay} providerTicker={providerTicker} dataError={marketData.error??historyError}/><p className="sub" style={{marginTop:24,lineHeight:1.7}}>Quote/history source: {marketData.source}. Fundamental source: {fundamentalSource}. {providerTicker?`Market-data symbol: ${providerTicker}. `:''}Historical chart range uses the full verified provider history when available. Current FX reference: {fxRate!==undefined?`${fxRate.toLocaleString('en-US',{maximumFractionDigits:4})} ${market.config.currencyCode}/USD.`:'unavailable'}</p></main></div>;
}
