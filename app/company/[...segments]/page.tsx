...
export async function generateMetadata({params}:{params:Promise<{segments:string[]}>}){
  ...
  return {title:company?`${company.name} (${company.ticker}) · ${market.config.countryName} · iStocks`:`Company · ${market.config.countryName} · iStocks`,description:company?`${company.name} (${company.ticker}) stock price, historical prices, market capitalization and financial information on iStocks.`:undefined};
}
...
export default async function CompanyPage({params}:{params:Promise<{segments:string[]}>}){
  ...
  const baseCompany=rows.find(x=>x.ticker.toUpperCase()===ticker);
  if(!baseCompany)notFound();

  const marketData=await getCompanyMarketData(baseCompany);
  ...
  return <div className="shell">
    ...
    <main className="detail">
      <CompanyIntelligence company={company} history={marketData.history} exchangeName={market.config.exchangeName} countryName={`${market.config.flag} ${market.config.countryName}`} currency={market.config.currencyCode} lastUpdated={lastUpdated} dataSource={marketData.source} delay={marketData.delay} providerTicker={marketData.providerTicker} dataError={marketData.error}/>
      <p className="sub" style={{marginTop:24,lineHeight:1.7}}>Quote/history source: {marketData.source}. {marketData.providerTicker?`Provider symbol: ${marketData.providerTicker}. `:''}Retrieved {formatTimestamp(marketData.retrievedAt,market.config.timezone)}. Financial metrics are only displayed when verified source data is available.</p>
    </main>
  </div>;
}
