...
export default function HomeClient({initialCountry='EG',initialTop=100,initialSector='All',initialSearch=''}:{initialCountry?:string;initialTop?:number;initialSector?:string;initialSearch?:string}){
  ...
  function updateUrl(next:Partial<{country:string;limit:number;industry:string;q:string}>){
    const params=new URLSearchParams(window.location.search);const nextCountry=next.country??country;const nextLimit=next.limit??limit;const nextIndustry=next.industry??industry;const nextQ=next.q??q;
    if(nextCountry==='EG')params.delete('country');else params.set('country',nextCountry);if(nextLimit===100)params.delete('top');else params.set('top',String(nextLimit));if(nextIndustry==='All')params.delete('sector');else params.set('sector',nextIndustry);if(nextQ.trim())params.set('search',nextQ);else params.delete('search');
  ...
  <div className="mobileTableHint">Swipe horizontally to view all ranking data →</div><div className="table"><div className="thead"><span>Rank</span><span>Company</span><span>Ticker</span><span>Sector</span><span>Price</span><span>Change</span><span>Market cap</span></div>{rows.map(c=><Link href={`/company/${country}/${c.ticker}`} className="row" key={`${country}-${c.ticker}`}><span className="rank">{c.rank?String(c.rank).padStart(2,'0'):'—'}</span><span><div className="company">{c.name}</div><div className="sub">{c.ticker} · {c.sector??'—'}</div><div className="mobileMarketCap"><b>{formatMarketCap(c.marketCapLocal,config.currencyCode)}</b><span>{formatMarketCap(c.marketCapUSD,'USD')}</span></div></span><span className="value muted">{c.ticker}</span><span className="value muted">{c.sector??'—'}</span><span className="value mobilePrice">{formatPrice(c.price,config.currencyCode)}</span><span className={`change ${c.changePercent===undefined?'muted':c.changePercent>0?'positive':c.changePercent<0?'negative':'muted'}`}>{c.changePercent===undefined?'—':`${c.changePercent>0?'+':''}${c.changePercent.toFixed(2)}%`}</span><span className="value desktopMarketCap"><b>{formatMarketCap(c.marketCapLocal,config.currencyCode)}</b><div className="sub">{formatMarketCap(c.marketCapUSD,'USD')}</div></span></Link>)}</div></section></main>
