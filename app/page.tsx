...
export async function generateMetadata({searchParams}:{searchParams:Promise<{country?:string}>}):Promise<Metadata>{await searchParams;return {title:'iStocks - North Africa',description:'North African stock-market rankings and company data.'};}
export default async function Home({searchParams}:{searchParams:Promise<{country?:string;top?:string;sector?:string;search?:string;exchange?:string}>}){
 ...
 let initialCompanies:Awaited<ReturnType<typeof getMarketCompanies>>=[];try{initialCompanies=await getMarketCompanies(selected)}catch{}
