...
export default function Home(){
  const [country,setCountry]=useState(()=>typeof window==='undefined'?'EG':validCountry(new URLSearchParams(window.location.search).get('country')));
  const [q,setQ]=useState(()=>typeof window==='undefined'?'':new URLSearchParams(window.location.search).get('search')??'');
  const [industry,setIndustry]=useState(()=>typeof window==='undefined'?'All':new URLSearchParams(window.location.search).get('sector')??'All');
  const [limit,setLimit]=useState(()=>typeof window==='undefined'?100:validLimit(new URLSearchParams(window.location.search).get('top')));

  useEffect(()=>{const onPop=()=>{const p=new URLSearchParams(window.location.search);setCountry(validCountry(p.get('country')));setLimit(validLimit(p.get('top')));setIndustry(p.get('sector')??'All');setQ(p.get('search')??'');};window.addEventListener('popstate',onPop);return()=>window.removeEventListener('popstate',onPop);},[]);

  const market=MARKET_REGISTRY[country]??MARKET_REGISTRY.EG;
  ...
  const filtered=useMemo(()=>allRows.filter(c=>(industry==='All'||c.sector===industry)&&(`${c.name} ${c.ticker}`.toLowerCase().includes(q.trim().toLowerCase()))),[allRows,q,industry]);
  ...
  function updateUrl(next:Partial<{country:string;limit:number;industry:string;q:string}>){
    const params=new URLSearchParams(window.location.search);
    const nextCountry=next.country??country; const nextLimit=next.limit??limit; const nextIndustry=next.industry??industry; const nextQ=next.q??q;
    if(nextCountry==='EG')params.delete('country');else params.set('country',nextCountry);
    if(nextLimit===100)params.delete('top');else params.set('top',String(nextLimit));
    if(nextIndustry==='All')params.delete('sector');else params.set('sector',nextIndustry);
    if(nextQ.trim())params.set('search',nextQ);else params.delete('search');
    const url=`${window.location.pathname}${params.toString()?`?${params}`:''}${window.location.hash}`;
    window.history.pushState({},'',url);
  }
  function changeCountry(next:string){setCountry(next);setIndustry('All');setQ('');setLimit(100);updateUrl({country:next,industry:'All',q:'',limit:100});}
  ...
  <section id="companies"><div className="toolbar"><div className="filters"><select aria-label="Ranking depth" className="select" value={limit} onChange={e=>changeLimit(+e.target.value)}>{TOP_LIMITS.map(n=><option key={n} value={n}>Top {n.toLocaleString()}</option>)}</select><select aria-label="Filter by sector" className="select" value={industry} onChange={e=>changeIndustry(e.target.value)}><option>All</option>{industries.map(x=><option key={x}>{x}</option>)}</select></div><input aria-label="Search companies" className="search" placeholder={`Search ${config.countryName} companies…`} value={q} onChange={e=>changeSearch(e.target.value)}/></div>{!q&&limit>summary.count&&<div className="searchNote">Showing all {summary.count} available {config.countryName} listed companies. No placeholder rows are added.</div>}{q&&filtered.length>limit&&<div className="searchNote">Showing all matching companies across the full available {config.countryName} universe.</div>}
  <div className="table"><div className="thead"><span>Rank</span><span>Company</span><span>Ticker</span><span>Sector</span><span>Price</span><span>Change</span><span>Market cap</span></div>{rows.map(c=><Link href={`/company/${country}/${c.ticker}`} className="row" key={`${country}-${c.ticker}`}><span className="rank">{c.rank?String(c.rank).padStart(2,'0'):'—'}</span><span><div className="company">{c.name}</div><div className="sub">{c.ticker} · {c.sector??'—'}</div><div className="mobileMarketCap"><b>{formatMarketCap(c.marketCapLocal,config.currencyCode)}</b><span>{formatMarketCap(c.marketCapUSD,'USD')}</span></div></span><span className="value muted">{c.ticker}</span><span className="value muted">{c.sector??'—'}</span><span className="value mobilePrice">{formatPrice(c.price,config.currencyCode)}</span><span className={`change ${c.changePercent===undefined?'muted':c.changePercent>0?'positive':c.changePercent<0?'negative':'muted'}`}>{c.changePercent===undefined?'—':`${c.changePercent>0?'+':''}${c.changePercent.toFixed(2)}%`}</span><span className="value desktopMarketCap"><b>{formatMarketCap(c.marketCapLocal,config.currencyCode)}</b><div className="sub">{formatMarketCap(c.marketCapUSD,'USD')}</div></span></Link>)}</div></section></main>
